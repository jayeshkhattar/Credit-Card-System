...
public class ManageRedemptionJPanel extends javax.swing.JPanel {
    ...
    public void populateCartTable()
    {
        ...
        Voucher voucher = (Voucher) merchantOffersTable.getValueAt(selectedRow, 0);
            ...
            row[2] = voucher.getPoints()*qnty;
            totalamt+=voucher.getPoints()*qnty;
...
//            pointsEarned += ord.getPointsEarned();
        ...
        {
            JOptionPane.showMessageDialog(null, "You do not have enough Points");
        }
    ...
    private void btnAddtoCartActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddtoCartActionPerformed
        ...
        orderList.add(order);
            ...
            Object [] row = new Object[3];
            row[0] = order.getVoucher();
            row[1] = qnty;
            row[2] = order.getVoucher().getPoints()*qnty;
            totalamt+=order.getVoucher().getPoints()*qnty;
            dtm2.addRow(row);
        ...
        for(Order ord : orderList) {
            ...
            ord.setPurchaseDate(dtf.format(now)+"");
            ord.setPointsEarned(ord.getPointsEarned()-(ord.getVoucher().getPoints()*qnty));
